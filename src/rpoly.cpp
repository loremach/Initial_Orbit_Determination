...
void roots(Matrix coef, Matrix & real, Matrix & im){
    double * op = vectorToArray(coef);
    double * zeror = vectorToArray(real);
    double * zeroi = vectorToArray(im);
    int degree = coef.n_column-1;
    rpoly(op, degree, zeror, zeroi);
    real = arrayToVector(zeror, degree);
    im = arrayToVector(zeroi, degree);
}
